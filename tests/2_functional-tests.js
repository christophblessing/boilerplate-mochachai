...
suite("Functional Tests", function () {
  ...
  test("Asynchronous test #example", function (done) {
    /** <= Pass a callback to the test function **/ setTimeout(function () {
  ...
  suite("Integration tests with chai-http", function () {
    ...
    suite('GET /hello?name=[name] => "hello [name]"', function () {
      ...
      test("Test GET /hello with your name", function (done) {
        ...
        chai
          .request(server) // 'server' is the Express App
          .get("/hello?name=foobar") /** <=== Put your name in the query **/
          .end(function (err, res) {
            // res is the response object
            ...
            assert.equal(
              res.text,
              "hello foobar" /** <==  Put your name here **/
            );
            done(); // Always call the 'done()' callback when finished.
          });
      });
        ...
        chai
          ...
          .end(function (err, res) {
            ...
            assert.equal(
              res.body.name,
              "Cristoforo",
              "response name should be Cristoforo"
            );
            assert.equal(
              res.body.surname,
              "Colombo",
              "response surname should be Colombo"
            );
      ...
      test('submit "surname" : "Colombo" - write your e2e test...', function (done) {
        ...
        browser.fill("surname", "Colombo").then(() => {
          browser.pressButton("submit", function () {
            ...
            // assert that status is OK 200
            browser.assert.success();
            // browser.assert that the text inside the element 'span#name' is 'Cristoforo'
            browser.assert.text("span#name", "Cristoforo");
            // browser.assert that the text inside the element 'span#surname' is 'Colombo'
            browser.assert.text("span#surname", "Colombo");
            // browser.assert that the element(s) 'span#dates' exist and their count is 1
            browser.assert.element("span#dates", 1);
      ...
      test('submit "surname" : "Vespucci" - write your e2e test...', function (done) {
        browser.fill("surname", "Vespucci").then(() => {
          browser.pressButton("submit", () => {
            ...
            // assert that status is OK 200
            browser.assert.success();
            // browser.assert that the text inside the element 'span#name' is 'Amerigo'
            browser.assert.text("span#name", "Amerigo");
            // browser.assert that the text inside the element 'span#surname' is 'Vespucci'
